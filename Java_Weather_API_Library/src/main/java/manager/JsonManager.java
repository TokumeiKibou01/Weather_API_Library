...
public class JsonManager {
    ...
    /**
     * Deserializedしたデータとして返す
     */
    public DeserializedJson getDeserializedJson() {
        return gson.fromJson(getRawElement(), type.getJsonClass());
    }
    ...
    public void updateJson() {
        try {
            ...
            try (BufferedWriter write = new BufferedWriter(new OutputStreamWriter(Files.newOutputStream(getFile().toPath()), StandardCharsets.UTF_8))) {
                ...
                System.out.print(getFile().getName() + "の更新に成功しました。");
            }
    ...
    public boolean createJson() {
        ...
        if (!getFile().exists()) {
            try (InputStream in = Main.class.getClassLoader().getResourceAsStream(type.name)) {
                Files.copy(in, new File(type.name).toPath());
                updateJson();
                status = true;
            } catch (IOException e) {
                throw new RuntimeException(e);
    ...
    public enum FileType {
        ...
        private final Class<? extends DeserializedJson> json_class;

        FileType(String name, Class<? extends DeserializedJson> json_class) {
            this.name = name;
        ...
        public Class<? extends DeserializedJson> getJsonClass() {
            return json_class;
        }
    }
